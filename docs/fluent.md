...
`Condition` is a list of conditions that're all connected with `AND` or `OR`. The magic is, that condition can be also another complex with different type (`AND` or `OR`).
...
To create condition in a simplier way, there are methods `whereAnd()`/`whereOr()`/`havingAnd()`/`havingOr()` on the `Query` that return a new `Condition` connected to a query.
